...
import segundum.usuarios.modelo.LugarDeRecogida;
...
import segundum.usuarios.repositorio.RepositorioProductosAdHoc;
...
public class ServicioProductos implements IServicioProductos {
	...
	public List<Producto> buscarProductos(String descripcion, String idCategoria, EstadoProducto estado,
		...
		if (idCategoria != null && !idCategoria.trim().isEmpty()) {
			...
			// Índice id -> Categoria
			java.util.List<segundum.usuarios.modelo.Categoria> todas = repositorioCategorias.getAll();
			java.util.Map<String, segundum.usuarios.modelo.Categoria> porId = new java.util.HashMap<>();
			for (segundum.usuarios.modelo.Categoria c : todas) {
				porId.put(c.getId(), c);
			}

			segundum.usuarios.modelo.Categoria raiz = porId.get(idCategoria);
			if (raiz != null) {
				// DFS por subcategorias
				java.util.Deque<segundum.usuarios.modelo.Categoria> pila = new java.util.ArrayDeque<>();
				pila.push(raiz);
				while (!pila.isEmpty()) {
					segundum.usuarios.modelo.Categoria actual = pila.pop();
					if (idsCategoriasValidas.add(actual.getId())) {
						java.util.List<segundum.usuarios.modelo.Categoria> subs = actual.getSubcategorias();
						if (subs != null) {
							for (segundum.usuarios.modelo.Categoria h : subs) {
								pila.push(h);
							}
						}
					}
